...
class ImageToMap:
    """
    Converts an image to a map representation by extracting walls, boxes, people, and disposal center.
    ...
    def detect_people(self) -> None:
        ...
        txt_people_position = "\tself._bombs_pos = ["
    ...
    def detect_disposal_center(self) -> None:
        """
        Detects the disposal center (red area) in the map image and prints its position and size.
        """
        # convert to hsv colorspace
        img_hsv = cv2.cvtColor(self._img_src, cv2.COLOR_BGR2HSV)
        ...
        mask_disposal_center = lower_mask + upper_mask

        # define kernel size
        kernel = np.ones((7, 7), np.uint8)

        # Remove unnecessary noise from mask
        mask_disposal_center = cv2.morphologyEx(mask_disposal_center,
                                              cv2.MORPH_CLOSE,
                                              kernel)
        mask_disposal_center = cv2.morphologyEx(mask_disposal_center,
                                              cv2.MORPH_OPEN,
        ...
        # find the contours
        contours, _ = cv2.findContours(mask_disposal_center,
                                       cv2.RETR_TREE,
        ...
        if contours:
            ...
            # draw contour
            mask_disposal_center_rbg = cv2.cvtColor(mask_disposal_center,
                                                  cv2.COLOR_GRAY2RGB)
            mask_disposal_center_rbg = cv2.drawContours(mask_disposal_center_rbg,
                                                      [cnt],
                                                      0,
                                                      (0, 255, 255),
                                                      2)
            ...
            mask_disposal_center_rbg = cv2.rectangle(mask_disposal_center_rbg,
                                                   (x_rec, y_rec),
                                                   (x_rec + w_rec, y_rec + h_rec),
                                                   (0, 255, 0), 2)

            # display the image with bounding rectangle drawn on it
            cv2.imshow("Bounding Rectangle", mask_disposal_center_rbg)
            cv2.waitKey(0)

            # self._disposal_center = DisposalCenter(size=(90, 170))
            # self._disposal_center_pos = ((-505, -285), 0)
            ...
            txt_rescue1 = ("\tself._disposal_center = DisposalCenter(size=({0:.0f}, "
                           "{1:.0f}))").format(w_rec, h_rec)
            txt_rescue2 = ("\tself._disposal_center_pos = (({0:.0f}, "
                           "{1:.0f}), 0)").format(x, y)
            print(txt_rescue1)
            print(txt_rescue2)
