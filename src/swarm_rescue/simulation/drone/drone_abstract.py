...
from swarm_rescue.simulation.drone.bomb_deployer import BombDeployer
...
from swarm_rescue.simulation.drone.ground_truth_guard import assert_ground_truth_allowed
...
class DroneAbstract(Agent):
    ...
    Example Usage
        ...
        grasped_bombs = drone.grasped_bombs()
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.bomb_deployer: Optional[BombDeployer] = None
        ...
        # Red-team submission: submitted binary exploration map (H x W).
        # When set, the simulation loop will end the round early.
        self._exploration_map_submission: Optional[np.ndarray] = None
    ...
    def set_competition_mode(self, enabled: bool) -> None:
        """Enable ground-truth API guards inside control() (evaluation runs)."""
        self._competition_mode = enabled
    ...
    def grasped_bombs(self):
        """
        Returns the bombs currently grasped by the drone.

        Returns:
            list: List of grasped bombs.
        """
        return self.grasper.grasped_bombs

    def carried_bombs_count(self) -> int:
        """
        Returns the number of bombs this drone can still place (red team).

        Returns:
            int: Remaining placement inventory, or 0 if deployer is disabled.
        """
        if self.bomb_deployer is not None and self.bomb_deployer.enabled:
            return self.bomb_deployer.inventory
        return 0

    def enable_bomb_deployer(self, the_map, initial_count: int = 1) -> None:
        """
        Enable bomb placement for red-team mode.

        Args:
            the_map: Map instance used to spawn placed bombs.
            initial_count: Bombs carried by this drone at mission start.
        """
        if self.bomb_deployer is None:
            self.bomb_deployer = BombDeployer(self.base)
            self.base.add_device(self.bomb_deployer)
            self.base.add_device(self.bomb_deployer.place_controller)
            pg = self.playground
            if pg is not None:
                pg.add(self.bomb_deployer, allow_overlapping=True)
                pg.add(self.bomb_deployer.place_controller, allow_overlapping=True)
        self.bomb_deployer.enable(the_map, initial_count)

    def submit_exploration_map(self, grid: np.ndarray) -> None:
        """
        Submit the predicted exploration map (binary 2D matrix) to end the round.

        This is a red-team API. The simulation will detect this submission and
        terminate the current round early.

        Args:
            grid: 2D array with shape (H, W). Non-zero values are treated as 1
                when saving to JSON.
        """
        self._exploration_map_submission = np.asarray(grid).copy()
    ...
    def true_position(self) -> np.ndarray:
        ...
        assert_ground_truth_allowed(
            "true_position", competition_mode=self._competition_mode
        )
    ...
    def true_angle(self) -> float:
        ...
        """
        assert_ground_truth_allowed(
            "true_angle", competition_mode=self._competition_mode
        )
    ...
    def true_velocity(self) -> np.ndarray:
        ...
        """
        assert_ground_truth_allowed(
            "true_velocity", competition_mode=self._competition_mode
        )
    ...
    def true_angular_velocity(self) -> float:
        ...
        """
        assert_ground_truth_allowed(
            "true_angular_velocity", competition_mode=self._competition_mode
        )
