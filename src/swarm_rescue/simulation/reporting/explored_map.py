...
from swarm_rescue.simulation.utils.utils import bresenham, circular_kernel


def _create_black_white_image(img_playground) -> np.ndarray:
    """
    Convert a color image to a black and white image for wall detection.

    Args:
        img_playground: The color image of the playground.
...
def fill_empty_blob_of_wall(map_img) -> np.ndarray:
    ...
    Args:
        map_img: Black and white wall map.
...
class ExploredMap:
    """
    Keeps track of explored areas by drones and computes exploration scores.
    ...
    Main functionalities
        ...
        area
    Attributes:
        _img_playground (np.ndarray): Color image of the playground.
        _map_playground (np.ndarray): Black and white map of the playground.
        _map_shape (Tuple[int, int]): Shape of the map.
        _map_explo_lines (np.ndarray): Map of points visited by drones.
        _map_explo_zones (np.ndarray): Map of zones explored by drones.
    ...
    def __init__(self):
        ...
        """
        # img_playground : colored image of the playground without bombs
    ...
    def _create_image_walls(self, playground: Playground) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The color image of the playground.
        """
        view = TopDownView(playground=playground, zoom=1)
        view.update_and_draw_in_framebuffer()
    ...
    def initialize_walls(self, playground: Playground) -> None:
        """
        From _img_playground, it creates a black and white image of the walls
        saved in _map_playground.
        Creates an image of the playground without drones and bombs.
    ...
    def update_drones(self, drones: List[DroneAbstract]) -> None:
        """
    ...
    def get_pretty_map_explo_lines(self) -> np.ndarray:
        """
        Returns a map with the explored lines highlighted.

        Returns:
            np.ndarray: Map with explored lines.
        """
    ...
    def _process_positions_bresenham(self) -> None:
        """
    ...
    def _compute_reachable_pixels(self) -> int:
        """
        Compute the number of reachable pixels in the map.

        Returns:
            int: Number of reachable pixels.
        """
    ...
    def truth_obstacle_grid(self) -> np.ndarray:
        """
        Return the full-scene obstacle render as a binary grid.

        ``1`` marks everything outside the largest connected free region of the
        rendered playground: walls, the disposal center, closed pockets and
        render artifacts alike. Map-submission scoring does not use this grid
        directly -- it scores against the walls-only grid built from the map's
        real wall entities (see ``place_score_manager`` / ``launcher``) -- so
        this is used as the canvas/fallback source and for legacy pretty maps.

        Returns:
            np.ndarray: Binary grid where 1 = obstacle (non-free), 0 = free space.
        """
        if not self.initialized:
            return np.zeros((0, 0), dtype=np.uint8)
        return (self._map_playground == 0).astype(np.uint8)
    ...
    def score(self) -> float:
        ...
        Returns:
            float: Exploration score (0 to 1).
        """
        if not self.initialized:
            return 0
